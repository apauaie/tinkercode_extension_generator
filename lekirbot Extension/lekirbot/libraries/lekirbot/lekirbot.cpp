...
lekirbot::lekirbot(unsigned char* pins,
...
void lekirbot::knightrider(int delayx)
{
  digitalWrite(led1, HIGH);
  delay(delayx);
...
int lekirbot::ultrasonic()
{


}

void lekirbot::RelaySwitch(int delayx)
{
...
void lekirbot::MazeRunner (int speed)
{
    ...
    {
      motortank("forward", "backward", speed, speed);
    }
...
void lekirbot::motortank(char *directionL,  int powerL, char *directionR,int powerR)
{
...
void lekirbot::motorsteer(char* robotdirection, int power)
{
...
void lekirbot::megablink(int del)
{
  digitalWrite(led1, LOW);
  digitalWrite(led2, LOW);
  digitalWrite(led3, LOW);
...
void lekirbot::LED(int l,char *stat)
{
...
int lekirbot::ir_sense(int l)
{
int sensordata= analogRead(l);
return sensordata;
}

int lekirbot::Pbutton()
{
...
// or if you just want the emitters on all the time and don't want to
// use an I/O pin to control it, use a value of 255 (QTR_NO_EMITTER_PIN).
void lekirbot::init(unsigned char* pins,
    unsigned char numSensors, unsigned char numSamplesPerSensor,
    unsigned char emitterPin)
{
    QTRSensors::init(pins, numSensors, emitterPin);
...
void lekirbot::readPrivate(unsigned int *sensor_values)
{
    unsigned char i, j;
...
void QTRSensors::lekirbottillJunction(int lajul, int lajur, int speedturn, int sensorc, int inByte, float kp, float kd,int linef,int maxSpeed, int forwarddelay) 
...
void QTRSensors::lekirbottillJunctionSimple(int laju, int inByte, float kp, float kd, int linef) 
...
void QTRSensors::lekirbotFinderW(int inByte, int inNext, int lajur, int lajul, float turn, int speedturn, float kp, float kd, int sensorc, int lajak, int dturn) // (type of junction, next robot action, robot speed, turn speed percentage)
{
...
void QTRSensors::lekirbotFinderB(int inByte, int inNext, int lajur, int lajul, float turn, int speedturn, float kp, float kd, int sensorc, int lajak, int dturn) // (type of junction, next robot action, robot speed, turn speed percentage)
{
...
void QTRSensors::lekirbotFinderBlack(int inByte, int inNext, int laju,int turnspeed, float kp, float kd,  int terus,int turndelay) // (type of junction, next robot action, robot speed, turn speed percentage)
{
...
void QTRSensors::lekirbotFinderWhite(int inByte, int inNext, int laju, int turnspeed, float kp, float kd, int terus, int turndelay) // (type of junction, next robot action, robot speed, turn speed percentage)
{
...
void QTRSensors::lekirbotDtracer(int lajur, int lajul, int speedturn, float kp, float kd, int sensorc, int linef, int dtracer) // (type of junction, next robot action, robot speed, turn speed percentage)
{
...
void QTRSensors::lekirbotflineW(int lajur, int lajul, int sensorc, int linef) // (right speed, left speed, sensor, line format)
{
...
void QTRSensors::lekirbotflineB(int lajur, int lajul, int sensorc, int linef) // (right speed, left speed, sensor, line format)
{
	sensorc =(calibratedMaximumOn[2]+calibratedMinimumOn[2])/2;
    position = readLine(sensorValues,1,linef); //0 black on white, 1 white on black
